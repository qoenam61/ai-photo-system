...
echo
echo "[5/9] 복원 자산 자동 보호 (DD-restored-asset-protection)"
...
echo
echo "[6/9] 신규 TRASH cleanup_queue 자동 등록 (grace 7일)"
...
echo
echo "[7/9] Layer 5 HDD cleanup (grace 만료 + verify PASS만)"
...
echo
echo "[8/9] 등급 폴더 정합 (reconcile)"
run_step "[8] reconcile_grade_folders" bash -c "
  set -o pipefail
  PYTHONPATH=. $POETRY run python scripts/reconcile_grade_folders.py 2>&1 | tail -8
"

echo
echo "[9/9] 등급별 view symlink 정합"
run_step "[9] build_grade_views" bash -c "
  set -o pipefail
