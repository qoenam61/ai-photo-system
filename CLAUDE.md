...
### iCloud 보존 등급 (사용자 명시 정정, 2026-05-05)

iCloud Photos 백업 = **고퀄 등급만** (BEST/EVENT/EVENT-L/MEMORY+ 4개).
나머지는 HDD 보존 only — iCloud 50GB 한도 절감.

| 등급 | 정의 | iCloud 동기 (Mac 앨범) |
|---|---|---|
| **BEST** | 얼굴 명확한 인물 / 풍경·사물 좋은 컷 | ✅ Mac 앨범 동기 → iCloud → iPhone |
| **EVENT** | 얼굴 명확한 사람 3+ 또는 행사 (얼굴 가시성 필수) | ✅ Mac 앨범 동기 |
| **EVENT-L** | 행사 영상(Long, ≥3초) + EVENT 그룹 dedup 강등 | ✅ Mac 앨범 동기 (결혼식 영상 등 보존 가치) |
| **MEMORY+** | 얼굴 보이는 사람 + 화질 OK | ✅ Mac 앨범 동기 |
| **FOOD** | 음식 사진 | ❌ HDD 보존 only |
| **MEMORY-** | 얼굴 보이는 사람 + 흐림 | ❌ HDD 보존 only |
| **NORMAL** | 사람 없는 카메라 사진 | ❌ HDD 보존 only |
| **TRASH** | 의미 없는 사진 + 중복 | ❌ Mac 정리 대상 (별도 cleanup_photos_mac.py) |

**Mac Photos 앨범 자동 동기** (`scripts/sync_photos_albums.py`):
- 4등급 (BEST/EVENT/EVENT-L/MEMORY+) Manual Album 자동 추가
