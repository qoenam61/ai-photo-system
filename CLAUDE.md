...
### ABCD 등급 세분화 (사용자 명시, 2026-05-11)

10등급 유지 + `sub_category` 컬럼 추가로 라벨 분리 (정책 분기는 grade로만).

| 등급 | sub_category 옵션 | 식별 |
|---|---|---|
| **BEST** | portrait / landscape / unknown | `face_count>0` / `camera_make 명시` / 그 외 |
| **EVENT+** | wedding / family / other | `folder_bulk` / `contains_child` / 성인 다수 |
| **EVENT-L+** | wedding_video / video / wedding / family / other | `folder_bulk_no_convert` 영상 / 일반 영상 / 본식 이미지 / 자녀 / 그 외 |
| **EVENT-** | video / other | `is_video` / 그 외 |
| **EVENT-L-** | video / wedding_archive / other | `is_video` / `folder_bulk` / 그 외 |
| **MEMORY+** | video / image | `is_video` / 그 외 |
| **MEMORY-** | portrait / image / video | `face_count>0` (백필) / 그 외 |
| **NORMAL** | image / family / other / video | `face_count` 기반 |
| **FOOD** | image | (영상 분리 X) |
| **TRASH** | duplicate / screenshot / short_video / llm_no_face / llm_judged / other | `grade_source` 기반 |
